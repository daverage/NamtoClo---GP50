...
**Measured (2026-08-30)**, one DI clip ("Hotrod - Guitar.wav") through a clean amp and an extreme-high-gain amp:

- **Clean (Fender Super Reverb)**: the GP-5/GP-50 output stays within ~0.5dB of Full A2's output across the full 30dB sweep, tapering from +0.46dB (softest) to -0.03dB (loudest). Small, likely inaudible on its own.
- **Extreme-high-gain (Fortin Meshuggah)**: **a real, large mismatch**. Full A2's output barely moves (~1.4dB total) across the 30dB input sweep -- the expected behavior for a heavily-compressed high-gain amp, output pinned near its ceiling regardless of pick attack. The GP-5/GP-50 conversion moves much more (~4.4dB) -- **it's noticeably under-compressed relative to the real amp**, worst at soft playing levels (nearly 6dB quieter than it should be at -24dB input, narrowing to ~2.7dB at 0dB and above). The ESR between the two outputs also grows monotonically with level (1.26 -> 1.52), reinforcing this is a real, level-dependent divergence, not noise.

**Caveat on the ESR number specifically**: it's a raw time-domain sample-wise metric (unlike the frequency-domain ratio loss used everywhere else in this codebase), so it's likely sensitive to any small latency/group-delay mismatch between the two independent render chains (`renderNamOnSignal` vs. `renderCloOnSignal`) -- treat its absolute value with caution. The RMS-level comparison is NOT subject to this confound (RMS is phase/timing-insensitive), so that's the trustworthy part of this finding, and the ESR's *trend* (monotonically growing with level, differently between the two amps) is still informative even if its absolute scale isn't directly interpretable.

**What this means**: the conversion pipeline gets a single-level fit right (that's what every other metric this session has been checking) but does not reproduce a high-gain amp's actual compression *character* across the dynamic range a player would use. This is plausibly the missing piece explaining why P/K search (which controls exactly this asymmetric-saturation/compression behavior) found nothing under the existing loss metric -- that metric was never evaluated against multi-level content, so it never had the chance to see this. Not yet investigated: whether this generalizes across more high-gain amps and DI clips, and whether a genuinely dynamics-aware fitting/loss approach (roadmap items 7/9, not yet attempted) could close this gap. Measurement only -- no fix attempted yet.
