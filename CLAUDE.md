...
- The reverse-engineered `fitPk()`/`fitAB()` core fitting logic is unchanged from the original Valeton-style reconstruction.
- **Final output-level match, corrected reference (2026-09-15):** both the GP-200 2048-tap path (`native_converter.cpp`, right after `refineCloBOnly`) and the GP-5/GP-50 512-tap path (same file, GP-5/GP-50 Tone Match block) rescale the winning Tone Match Block B with a pure linear gain so its rendered RMS on the Tone Match analysis clip matches a reference level — a real, user-reported bug ("Tone Match output far quieter than without Tone Match") traced to that reference being the raw NAM render (Full A2) instead of the plain (non-Tone-Match) conversion's own rendered output. Those two are NOT the same level: the plain conversion is already tuned louder than a literal Full A2 render (the same gain-calibration gap the dynamics-aware P/K search below ran into), so matching Tone Match to Full A2 made it come out 10-19dB quieter than the plain conversion of the same NAM on real converted files (measured directly, not estimated). Both paths now match Tone Match's level to the PLAIN conversion's own render on the same clip instead, so this step only ever corrects TONE, never overall loudness relative to what a non-Tone-Match conversion of the same NAM already sounds like. `corrective_ir.cpp`'s new `scaleClo2048BlockB()` does the plain-linear-scale write-back for the 2048-tap CLO (deliberately NOT reusing `applyCorrectiveIrToClo`'s automatic rmsGain normalization, which forces Block B back to the ORIGINAL pre-Tone-Match Block B's RMS and would silently cancel out any gain this step tries to apply).
