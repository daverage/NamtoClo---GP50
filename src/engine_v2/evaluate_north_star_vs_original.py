...
def read_compact_clo(path: Path) -> CompactClo:
    ...
    pre = np.array(
        [struct.unpack_from("<d", data, off)[0] for off in (0x18, 0x20, 0x28, 0x30, 0x38)],
        dtype=np.float64,
    )
    post = np.array(
        [struct.unpack_from("<d", data, off)[0] for off in (0x40, 0x48, 0x50, 0x58, 0x60)],
        dtype=np.float64,
    )
...
def render_compact_clo(x: np.ndarray, clo: CompactClo) -> np.ndarray:
    """Exact device-style compact CLO render using coefficients embedded in the file."""
...
def select_shared_real_material(
    ...
    """Choose identical underlying performances for every compared NAM."""
...
def _discover_clo(root: Path, pair: Pair, kind: str) -> Path:
    ...
    if len(candidates) != 1:
        names = "\n  ".join(str(p) for p in candidates[:10]) or "<none>"
        raise RuntimeError(
...
def _best_lag(pred: np.ndarray, target: np.ndarray, maxlag: int = 256) -> int:
    ...
    ts = t[::dec]
    ml = max(1, maxlag // dec)
    nf = 1 << (max(1, len(ps) + len(ts) - 1) - 1).bit_length()
    corr = np.fft.irfft(
...
def _aligned(pred: np.ndarray, target: np.ndarray):
    ...
    if lag >= 0:
        return pred[lag:], target[: n - lag], lag
    return pred[: n + lag], target[-lag:], lag
...
def _welch_power(x: np.ndarray, nfft: int = 65536, hop: int = 16384):
    x = np.asarray(x, dtype=np.float64)
    if len(x) == 0:
        return np.fft.rfftfreq(nfft, 1.0 / SR), np.zeros(nfft // 2 + 1)
    win = np.hanning(nfft)
    ...
    if len(x) < nfft:
        ...
        for start in range(0, len(x) - nfft + 1, hop):
            X = np.fft.rfft(x[start : start + nfft] * win)
            acc += np.abs(X) ** 2 / norm
            frames += 1
            last_start = start
        if last_start is None or last_start + nfft < len(x):
            z = np.zeros(nfft, dtype=np.float64)
            tail = x[max(0, len(x) - nfft) :]
            z[-len(tail) :] = tail
            X = np.fft.rfft(z * win)
            acc += np.abs(X) ** 2 / norm
            frames += 1
...
def _spectral_views(pred: np.ndarray, target: np.ndarray) -> dict:
    ...
    target_rms = _rms(t)
    pred_rms = _rms(p)
    gain_matched = p * (target_rms / max(pred_rms, 1e-15))

    freqs, target_power = _welch_power(t)
    _, pred_power = _welch_power(p)
    _, gain_power = _welch_power(gain_matched)

    edges = np.geomspace(20.0, 20000.0, 145)
    curve = []
    target_bands = []
    ...
    equal_rmse = (
        math.sqrt(
            float(np.mean([r["gain_matched_error_db"] ** 2 for r in confident]))
        )
        if confident
        else float("nan")
    )
...
def _tail_indices(x: np.ndarray, frame: int = 4096):
    x = np.asarray(x, dtype=np.float64)
    rows = []
    levels = []
...
def _concat_ranges(x: np.ndarray, ranges) -> np.ndarray:
    if not ranges:
        return np.zeros(0, dtype=np.float64)
    return np.concatenate([np.asarray(x[a:b], dtype=np.float64) for a, b in ranges])
...
def _tail_metrics(x, target, pred) -> dict | None:
    ...
    if sum(b - a for a, b in ranges) < int(0.25 * SR):
        return None
    tail_input = _concat_ranges(x, ranges)
    tail_target = _concat_ranges(target, ranges)
    tail_pred = _concat_ranges(pred, ranges)
    basic = _pair_metrics(tail_pred, tail_target)
    spec = _spectral_views(tail_pred, tail_target)
    return {
        "seconds": len(tail_target) / float(SR),
        "frames": len(ranges),
        "esr": basic["esr"],
        ...
        "curve": spec["curve"],
        "bands": spec["bands"],
        "nam_nonlinear_residual_db": _linear_residual_db(tail_input, tail_target),
        "clo_nonlinear_residual_db": _linear_residual_db(tail_input, tail_pred),
    }
...
def _aggregate_curve(per_clip_specs: list[dict], system_key: str, field: str) -> list[dict]:
...
def _summary_metrics(clip_rows, system: str, specs: list[dict]) -> dict:
    ...
    return {
        ...
        "mean_abs_level_error_db": _mean(
            [{"v": abs(float(r[prefix + "level_error_db"]))} for r in clip_rows],
            "v",
        ),
        "mean_signed_level_error_db": _mean(clip_rows, prefix + "level_error_db"),
        "mean_gain_matched_esr": _mean(clip_rows, prefix + "gain_matched_esr"),
        "mean_gain_matched_spectral_rmse_db": float(
            np.mean([s[system]["gain_matched_spectral_rmse_db"] for s in specs])
        ),
        "mean_gain_matched_spectral_energy_weighted_rmse_db": float(
            np.mean(
                [
                    s[system]["gain_matched_spectral_energy_weighted_rmse_db"]
                    for s in specs
                ]
            )
        ),
...
def evaluate_model(
    ...
    for clip_index, pair in enumerate(pairs):
        ...
        row = {
            ...
            "original_tail_esr": original_tail["esr"] if original_tail else None,
            "original_tail_gain_matched_esr": (
                original_tail["gain_matched_esr"] if original_tail else None
            ),
            "original_tail_spectral_rmse_db": (
                original_tail["gain_matched_spectral_rmse_db"] if original_tail else None
            ),
            "original_tail_nonlinear_residual_delta_db": (
                original_tail["clo_nonlinear_residual_db"]
                - original_tail["nam_nonlinear_residual_db"]
                if original_tail
                else None
            ),
            ...
            "north_tail_spectral_rmse_db": (
                north_tail["gain_matched_spectral_rmse_db"] if north_tail else None
            ),
            "north_tail_nonlinear_residual_delta_db": (
                north_tail["clo_nonlinear_residual_db"]
                - north_tail["nam_nonlinear_residual_db"]
                if north_tail
                else None
            ),
            "tail_seconds": max(
    ...
    original_abs = _aggregate_curve(spectra, "original", "absolute_error_db")
    north_abs = _aggregate_curve(spectra, "north", "absolute_error_db")
    original_gm = _aggregate_curve(spectra, "original", "gain_matched_error_db")
        ...
        spectrum_rows.append(
            {
                ...
                "original_gain_matched_error_db": og["gain_matched_error_db"],
        ...
        band_rows.append(
            {
                ...
                "north_gain_matched_error_db": north_band["gain_matched_error_db"],
                "confident_clips": min(
    ...
    if tail_spectra:
        ...
        tail_north_abs = _aggregate_curve(tail_spectra, "north", "absolute_error_db")
    ...
    summary = {
        ...
        "analysis": {
            "sample_rate": SR,
            "spectrum_hz": [20.0, 20000.0],
            "dense_log_bands": 144,
            "welch_fft": 65536,
            "teacher_confidence_floor_db": -70.0,
            "broad_band_confidence_floor_db": -60.0,
            "tail_definition": (
                "non-overlapping 4096-sample input frames 20-50 dB below each clip's "
                "95th-percentile input RMS, excluding frames below -85 dBFS"
            ),
            "benchmark_only": True,
...
def _print_model_summary(summary: dict) -> None:
    ...
    if original["mean_tail_esr"] is not None and north["mean_tail_esr"] is not None:
        print(
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Three-way held-out evaluator: authoritative NAMCore teacher target vs "
            "ordinary NamToClo compact GP5/GP50 CLO vs frozen North Star v2 CLO."
        )
    )
    ...
    parser.add_argument(
        "--original-root",
        required=True,
        help="Directory containing ordinary NamToClo *_NATIVE_GP5GP50_512.clo files.",
    )
    parser.add_argument(
        "--north-star-root",
        required=True,
        help="North Star v2 output root containing per-model distilled.clo files.",
    )
    ...
    if len(keys) < 2:
        raise SystemExit(
            "Need at least two matched NAM models for a controlled shared-material comparison"
        )
    ...
    print(
        "spectrum: 20 Hz-20 kHz, 144 log bands + broad summaries; "
        "low-energy teacher bins flagged"
    )
    ...
    root_summary = {
        "method": "north-star-v2-three-way-heldout-evaluation-v1",
