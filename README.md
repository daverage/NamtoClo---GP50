...
1. [Download the latest release](../../releases) and extract the zip, or build from source (see [Build from source](#build-from-source)).
2. Keep `nam_input_wav.wav` in the **same folder as `NamToClo.exe`** (already true if you extracted a release zip).
...
The easiest way to get a working copy is the [**Releases** page](../../releases) — download
the latest `NamToClo-Windows-x64.zip`, extract it, and run `NamToClo.exe` from inside the
extracted folder. Every release zip already contains everything needed to run:

```text
NamToClo.exe
nam_input_wav.wav
reference_clips/        -- bundled Tone Match "Auto"/named reference clips
README.md, LICENSE, THIRD_PARTY.md, and third-party LICENSE files
```

GP-200, GP-5 and GP-50 upload support is built into the executable; no separate MIDI runtime or Valeton Suite installation is required. `reference_clips/` is only needed for Tone Match's **Auto** or named (Clean/Moderate/High Gain/Bass) reference modes — **Default** and **Custom WAV...** work without it.

If building from source instead of downloading a release, see [Build from source](#build-from-source) below — `cmake --install` stages the same complete folder that the release zip contains.
...
(`build/Release/NamToClo.exe`), with `reference_clips/` already copied next to it as a
post-build step. To stage a complete, self-contained distributable folder (the same
layout the release zip uses — exe, `reference_clips/`, licenses, docs) in one step:

```powershell
cmake --install build --config Release --prefix dist/NamToClo
Copy-Item nam_input_wav.wav dist/NamToClo/
```
...
- **Multi-level Block B solve for GP-5/GP-50.** The Tone Match step now also tries
  ...
  as a real (if modest) fidelity improvement over a single-operating-point solve. An
  experimental gain-shaper search that goes further was tried and found, via real
  hardware listening, to make patches quieter without actually cleaning up distortion;
  it ships disabled with no GUI control. See
  [GP-5/GP-50 Tone Match method](#gp-5gp-50-tone-match-method) below.
