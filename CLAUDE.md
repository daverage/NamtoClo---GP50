...
### Definitive official-vs-ours benchmark (resources/GP50_SnapTone_Conversion_Benchmark_Plan_v2.md, Priority 1) — 6 amps: official leads on dynamics, ours leads on spectral fidelity, and the extreme-gain result is the one that matters most

Every quality claim earlier in this document is relative to Full A2 (the NAM's own neural render) — never checked against what Valeton's own official SnapTone conversion actually achieves on the same NAM. `resources/GP50_SnapTone_Conversion_Benchmark_Plan_v2.md` (a separate reverse-engineering writeup of Valeton's `startClone()` fitter, not authored this session) lays out the definitive three-way benchmark this gap calls for. `native_converter.hpp`'s `runOfficialSnaptoneBenchmark()` (headless CLI `--official-benchmark <nam> <officialSnapClo> <diClipWav> <outputCsv> <heldOutClip1> [heldOutClip2...]`) implements it: given a real official SnapTone CLO for `inputNam`, it converts the SAME NAM with our own pipeline, then renders Full A2, the official CLO, and ours through the SAME software renderer (`renderCloOnSignal`, already tap-count-agnostic — it reads A/B counts from the VTSI header, so it works unchanged on the official file's GP-200-native 128/2048-tap layout) to remove hardware/capture confounds per the plan's section 15. It auto-picks the matching-architecture candidate (`gp2001024` for a 2048-tap B, `gp5gp50Compact` for a 512-tap B) so the comparison is apples-to-apples. Reports the same six-level relative-dynamics sweep used throughout this document's dynamics work, plus held-out real-clip waveform ESR vs Full A2.

**Measured (2026-08-31), 6 real official SnapTone conversions available** (`test_assets/snaptone_conversions/*.clo`, user-supplied, all GP-200-format 8840-byte VTSI files), guitar DI clip = "Hotrod - Guitar.wav" (bass amp used "Downtown - Bass.wav") for the dynamics sweep, 4 disjoint held-out clips per amp (guitar: Groove Thrash/Fast Thrash/Mayer/Cream; bass: Drivin'/Frogger/Garden/Rollin') for spectral/waveform fidelity — see `test_assets/quality_results/OfficialBenchmark/*.csv`:

| NAM | Category | Dynamics RMS err: official vs ours | Winner | Held-out mean ESR: official vs ours | Winner |
|---|---|---|---|---|---|
| Fender Super Reverb 1977 | Clean | 0.160 vs 0.200 dB | Official | 6.66 vs 11.53 | Official |
| Ampeg SVT Classic MD421 | Bass | 0.327 vs 0.330 dB | Official (~tied) | 19.41 vs 21.33 | Official |
| JCM800 2203 Modified (EdgeOfBreakup) | Edge of breakup | 0.539 vs 0.416 dB | Ours | 8.40 vs 5.79 | Ours |
| Marshall 1962 Bluesbreaker JTM45 | Crunch | 1.29 vs 1.06 dB | Ours | 9.16 vs 8.18 | Ours |
| JCM800 2203 Modified (HighGain) | High gain | 3.27 vs 4.43 dB | Official | 20.37 vs 13.57 | Ours |
| Fortin Meshuggah | Extreme gain | 1.24 vs 2.50 dB | Official (2x) | 18.72 vs 18.53 | ~tied |

**Verdict: genuinely mixed — official wins dynamics on 4/6, ours wins held-out spectral fidelity on 3/6 (tied on a 4th).** Not the clean "ours is better" result this session's other measurements might suggest, and not a clean "official is better" story either — the two converters are trading wins depending on what's measured, not one dominating the other. On dynamics tracking specifically — the exact thing Step 1/Step 2's P/K search above was built to fix — the OFFICIAL conversion beats our current shipped conversion on 4 of 6 real amps (clean, bass, high-gain, extreme-gain), with the extreme-gain case (Meshuggah) the largest gap of any amp tested: our shipped conversion's dynamics error is roughly 2x official's here.

**This Meshuggah result is the most decision-relevant number in this whole benchmark.** CLAUDE.md's Step 2 section already measured the P/K dynamics search taking Meshuggah's held-out dynamics RMS error from ~1.08 down to ~0.05 (a -95% reduction) against our own unoptimized baseline. Official's dynamics error here (1.24, different DI clip/methodology than Step 2's benchmark-clip scoring, so not a bit-for-bit comparable number, but the same order of magnitude as Step 2's *pre-search* baseline) sits well above where Step 2's search already showed our OWN pipeline can land. In other words: our shipped conversion currently trails official on the amp category where our unshipped optimization work claims its biggest win — meaning Step 2, if wired into production and reverified directly against this official file (not just against our own baseline), most plausibly would not just close the gap but overtake official specifically on extreme-gain material. This has not been directly verified yet (`runOfficialSnaptoneBenchmark` doesn't currently know how to substitute a Step-2-optimized candidate for the shipped one) — it is the highest-value next experiment this benchmark points to, not yet run.

Held-out ESR favors ours on 3 of 6 (tied on Meshuggah), but the absolute magnitudes (6-28) should be read with the same caution CLAUDE.md's dynamic-range section already flagged: this is a raw time-domain, phase-sensitive metric, and here Full A2 (NAMCore) and both CLO candidates (Pre->A->PK->Post->B) are structurally very different render chains with no explicit time-alignment step. The official-vs-ours comparison is still meaningful (the same confound applies to both, since both go through the identical `renderCloOnSignal` chain), but don't read the raw numbers as literal percentage errors — the dynamics RMS numbers (phase/timing-insensitive, same relative-anchoring convention used throughout this document) are the trustworthy half of this table.

**Sample size is 6 NAMs** — still small. Do not treat "official wins dynamics 4/6" as a hard rule, but it is now a repeated pattern across clean/bass/high-gain/extreme-gain, not a single outlier. If more real official SnapTone conversions become available, re-running `--official-benchmark` against them continues to be valuable.

**Direct verification (2026-08-31): the Step 2-optimized candidate beats official on Meshuggah, decisively.** `runOfficialSnaptoneBenchmark` gained optional `trainDiClipWav`/`selectionDiClipWav` parameters (headless CLI: `--official-benchmark <nam> <officialSnapClo> <diClipWav> <outputCsv> <trainClip|-> <selectionClip|-> <lambda> <heldOutClip1> [...]`) — when supplied, it runs Step 2's `searchPkForDynamics` against our own conversion (using `diClipWav` itself as the search's disjoint benchmark clip, matching the discipline every other search in this codebase uses) and scores the resulting unshipped optimized candidate against the SAME official file and Full A2 reference the shipped candidate is scored against. Run once, on Meshuggah specifically (train=Groove Thrash, selection=Fast Thrash, benchmark=Hotrod, held-out=Mayer/Cream, all disjoint) — see `test_assets/quality_results/OfficialBenchmark/fortin_meshuggah_optimized.csv`:

| Candidate | Dynamics RMS err (vs Full A2) | Held-out mean ESR (vs Full A2) |
|---|---:|---:|
| Official | 1.238 dB | 18.25 |
| Ours (shipped) | 2.504 dB | 19.99 |
| **Ours (Step 2-optimized, unshipped)** | **0.407 dB** | **0.50** |

The optimized candidate beats official's dynamics error by ~3x (not just closes the gap our shipped conversion had) and drops held-out waveform ESR by roughly 40x versus both official and our own shipped candidate. The dynamics-RMS number is the trustworthy half of this (phase/timing-insensitive, same relative-anchoring convention validated throughout this document) and is directly consistent with Step 2's original finding for this same NAM (CLAUDE.md's Step 2 section: -95% benchmark RMS reduction against our own baseline, on a different train/benchmark clip pairing) — this run adds the missing piece, confirming that improvement doesn't just close the gap to official, it clears it by a wide margin. The ESR number's *magnitude* should be read cautiously per the caveat above (raw time-domain, phase-sensitive metric) — but a ~40x drop while official/ours/optimized all share the literal same render chain (`renderCloOnSignal`/`renderCloWithOverrideOnSignal`, only B/PK coefficients differ) is a real, large, same-chain effect, not an artifact of comparing different DSP structures.

**Replicated (2026-08-31) on a second extreme-gain amp, Bogner Uberschall MKII** — the single worst dynamics-error case found anywhere in this document (the original finding, CLAUDE.md's dynamic-range section: 7.29dB max relative error). Same train=Groove Thrash/selection=Fast Thrash/benchmark=Hotrod/held-out=Mayer+Cream setup, lambda=0.3 — see `test_assets/quality_results/OfficialBenchmark/bogner_uberschall_optimized.csv`:

| Candidate | Dynamics RMS err (vs Full A2) | Held-out mean ESR (vs Full A2) |
|---|---:|---:|
| Official | 3.00 dB | 18.06 |
| Ours (shipped) | 4.46 dB | 14.57 |
| **Ours (Step 2-optimized, unshipped)** | **0.75 dB** | **0.62** |

Same pattern as Meshuggah: official already beats our shipped conversion on dynamics here (3.00 vs 4.46), and the Step 2-optimized candidate beats official by ~4x on dynamics and ~24-29x on held-out ESR, on the single worst amp this entire session measured. Two extreme-gain amps now show the identical qualitative result (optimized clears official by a wide margin, not just closes the shipped-vs-official gap), which is a real replication, not a restated single case — but it is still only extreme-gain amps; the pattern has not yet been checked on an amp where OURS already wins (e.g. JCM800 Edge of Breakup, Marshall Bluesbreaker) or on clean/bass amps where the absolute error is already small and Step 2's own earlier measurement (CLAUDE.md's Step 2 section) found it correctly declines to change anything on Fender Super Reverb. Before treating "Step 2 beats official" as general: check a case in each of those remaining buckets, not just more extreme-gain amps.
