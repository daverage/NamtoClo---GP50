...
def run_model(ps,out,args):
    ...
    for role,aud in [('fit',fit),('selection',sel),('benchmark',bench)]:
        if not aud:continue
        x,y,p=aud[0];pred=render_full(x,a,best.pk,best.b)
    ...
    report={'method':'clean-sheet-varpro-v1','model_name':name,'model_key':ps[0].model_key,'tone_id':ps[0].tone_id,'model_id':ps[0].model_id,'A128':a,'pk':best.pk,'B512_internal':best.b,'B512_storage':best.b*4,'fit_metrics':best.fit,'selection_metrics':best.selection,'benchmark_metrics':bm,'elapsed_seconds':time.monotonic()-t,'clo_path':str(clo),'preview_tasks':previews}
    dump(d/'report.json',report);print('CLO:',clo);return report
