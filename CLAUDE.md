...
**Still one (NAM, submodel) combination** — the same caution every early single-case finding in this document carries before being trusted as general. This is the single most encouraging data point for wiring Step 2 into production found so far, but it is one amp, one train/selection/benchmark clip triple, one lambda (0.3, the value used throughout Step 2's original measurement). Before treating this as settled: re-run with the other high/extreme-gain official files as they become available (Bogner Uberschall, PRS Archon, JCM800 HighGain, Ampeg SVT — official already beats our shipped conversion on 2 of these 4), and check whether the same optimized-beats-official pattern holds specifically on the amps where official currently wins, not just where ours already wins.
