...
### Definitive official-vs-ours benchmark (resources/GP50_SnapTone_Conversion_Benchmark_Plan_v2.md, Priority 1) — first 3 results: mixed, not a clean win for either side
...
**Measured (2026-08-31), the first 3 real official SnapTone conversions available** (`test_assets/snaptone_conversions/*_Snap.clo`, user-supplied, all GP-200-format 8840-byte VTSI files), DI clip = "Hotrod - Guitar.wav" for the dynamics sweep, 4 disjoint held-out clips (Groove Thrash, Fast Thrash, Mayer, Cream) for spectral/waveform fidelity — see `test_assets/quality_results/OfficialBenchmark/*.csv`:
...
**Verdict: mixed, not the clean "ours is better" result this session's other measurements might suggest.** On dynamics tracking specifically — the exact thing Step 1/Step 2's P/K search above was built to fix — the OFFICIAL conversion beats our current shipped conversion on 2 of the 3 real amps tested (clean and high-gain), not the other way around. Only on the crunch amp does ours win both metrics. This reframes Step 2: its large measured win (50-95% RMS dynamics-error reduction, see above) is against our OWN unoptimized baseline, and this result shows that baseline can trail Valeton's own official fitter on real hardware-equivalent output — so Step 2 is better understood as *closing a real gap toward Valeton's quality*, not polishing a baseline that was already ahead. If anything this strengthens the case for eventually verifying Step 2 against official references specifically (does the P/K-optimized candidate actually beat the official file, not just our own baseline?), rather than undermining the dynamics work.

Held-out ESR favors ours on 2 of 3, but the absolute magnitudes (6-28) should be read with the same caution CLAUDE.md's dynamic-range section already flagged: this is a raw time-domain, phase-sensitive metric, and here Full A2 (NAMCore) and both CLO candidates (Pre->A->PK->Post->B) are structurally very different render chains with no explicit time-alignment step. The official-vs-ours comparison is still meaningful (the same confound applies to both, since both go through the identical `renderCloOnSignal` chain), but don't read the raw numbers as "11% vs 20% error" in any literal sense — the dynamics RMS numbers (phase/timing-insensitive, same relative-anchoring convention used throughout this document) are the trustworthy half of this table.

**Sample size is 3 NAMs** — the same caution every small-corpus finding in this document carries. Do not generalize "official wins on dynamics" as a rule from 2 of 3 cases. If more real official SnapTone conversions become available, re-running `--official-benchmark` against them is the highest-value next step for deciding whether/when Step 2's P/K search is actually ready to wire into production. Comparative/measurement only — this tool does not modify or replace anything shipped.
