...
**Sample size is 6 NAMs** — still small. Do not treat "official wins dynamics 4/6" as a hard rule, but it is now a repeated pattern across clean/bass/high-gain/extreme-gain, not a single outlier. If more real official SnapTone conversions become available, re-running `--official-benchmark` against them continues to be valuable, but the highest-value next step is now specifically: extend the comparison to score a Step-2-optimized (or production-wired) candidate against these same official files, to check whether the dynamics-aware fitting work actually closes or reverses the official gap it just revealed. Comparative/measurement only — this tool does not modify or replace anything shipped.
