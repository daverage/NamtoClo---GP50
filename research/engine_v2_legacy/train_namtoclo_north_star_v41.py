...
METHOD = "north-star-teacher-student-varpro-v4.1-t3k-multilevel-converged"


def run_model(pairs, nam, renderer, renderer_sha, renderer_commit, stimulus_path, out, args):
    name = pairs[0].model_name
    print(f"\n=== NORTH STAR v4.1 STIMULUS + CONVERGENCE: {name} ===")

    # Guitar remains post-fit comparison only. These are the same deterministic
    # development partitions used for V3/V4 comparison, not fresh benchmark data.
    ...
    fit, stimulus_manifest = prepare_multilevel_teacher_audio(
        ...
        cache_root=out / "_v41_teacher_cache",
    ...
    print(
        f"v4.1 FIT evidence={evidence.virtual_examples} complete waveform pairs; "
    ...
    print(
        ...
        f"FIT-only trajectories (max {args.rounds} rounds), then selected-basin polish "
        f"at A={FINE_A_STEP_DB:g}dB P/K={FINE_PK_LOG_STEP:g} for <= {args.polish_cycles} cycles"
    )

    started = time.monotonic()
    search_info = {}
    best = distill_north_star_v41(
        fit,
    ...
    # Intentionally unchanged from V4. Output-calibration behavior is a separate
    # experiment so V4.1 isolates convergence only.
    ...
    for role in ("v3_fit_real", "v3_selection_real", "v3_benchmark_real"):
        ...
        bits = [f"V4.1 ESR={vm.esr:.4f} level={vm.signed_level_db:+.2f} dB"]
    ...
    clo = directory / f"NSV41__{pairs[0].model_id}__{safe(name)}.clo"
    ...
    report = {
        ...
        "experiment_document": "NORTH_STAR_V4_1_CONVERGENCE_EXPERIMENT.md",
        "model_name": name,
        ...
        "optimization": {
            ...
            "v41_only_change": "repeat fine A/P-K sweeps on the V4-selected basin until convergence/safety cap",
        ...
        "guitar_comparison_only": {
            "note": (
                "These real-guitar clips are evaluated only after V4.1 coefficients are frozen. "
                "They do not affect A/P-K/B or output calibration. Existing V3 partitions are "
                "reused solely for paired development comparison and are not fresh held-out evidence."
            ),
            "material": {
                role: [_pair_row(p) for p in ps]
                for role, ps in comparison_pairs.items()
            },
            "v41_metrics": guitar_metrics,
            "v4_reference": v4_ref,
            "v3_reference": v3_ref,
        },
        "A128": a,
    ...
    if selected:
        print(
            ...
            f"-> V4.1 ESR={selected.get('v41_fit_evidence_esr', float('nan')):.6g} "
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "EngineV2 North Star v4.1: run the exact V4 TONE3000 multilevel "
            "stimulus-only fitter, then repeatedly polish the V4-selected A/P-K "
            "basin at the established fine step floors until convergence. Real "
    ...
    p.add_argument("--output", default="~/NamtoCloArchive/NamtoCloNorthStarV4_1_Converged")
    ...
    p.add_argument("--v3-root", default="~/NamtoCloArchive/NamtoCloNorthStarV3_SearchRobust")
    ...
    if args.polish_cycles < 1:
        raise SystemExit("--polish-cycles must be >= 1")
    ...
    print("v4.1 stimulus:", stimulus_path)
    print("v4.1 stimulus sha256:", sha256_file(stimulus_path))
