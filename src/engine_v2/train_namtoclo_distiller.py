...
def _calibrate_output_gain(audio,a,pk,b,max_abs_db=12.0,peak_margin_db=0.25):
    ...
    scaled=b*(10.0**(gain_db/20.0));after=score(audio,a,pk,scaled)
    info={
...
def run_model(ps,out,args):
    ...
    print(f'level sweeps fit={len(lfg)} selection={len(lsg)} benchmark={len(lbg)} weight={args.level_response_weight:g}')
    t=time.monotonic();best=distill(
        fit,sel,args.a_controls,args.rounds,pause_ms=args.yield_ms,
        level_fit=level_fit,level_sel=level_sel,level_weight=args.level_response_weight
    ...
    report={
        ...
        'optimizer_level_response':{'fit_rmse_db':best.fit_level_response_db,'selection_rmse_db':best.selection_level_response_db,'weight':args.level_response_weight},
...
def main():
    ...
    dump(out/'summary.json',{'results':results,'failures':fail,'cpu_controls':{'threads':a.threads,'yield_ms':a.yield_ms},'level_controls':{'groups':a.level_groups,'weight':a.level_response_weight}});return 1 if fail else 0
