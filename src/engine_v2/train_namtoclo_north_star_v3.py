...
from distiller_v2_north_star_v2 import build_fit_evidence
from distiller_v2_north_star_v3 import (
    DEFAULT_MAX_ROUNDS,
    DEFAULT_MULTISTARTS,
    MAX_MULTISTARTS,
    distill_north_star_v3,
)
...
METHOD = "north-star-teacher-student-varpro-v3-search-robust"
...
def run_model(pairs, out, args):
    ...
    print(
        f"v3 FIT evidence={evidence.virtual_examples} normalized windows; "
        "same v2 evidence/objective, search mechanics only changed"
    )
    ...
    print(
        ...
        f"FIT trajectories (max {args.rounds} rounds); SELECTION chooses checkpoints only"
    )

    # Matched real-guitar sweeps remain diagnostics only, exactly as in v2.
    ...
    # Keep NSV2__ filename compatibility with the existing analyzers. Provenance
    # is carried by report.method and report.optimization.search.
    clo = directory / f"NSV2__{pairs[0].model_id}__{safe(name)}.clo"
    ...
    report = {
        ...
        "optimization": {
            ...
            "selection_role": (
                "mean aligned ESR across disjoint real SELECTION examples chooses "
                "among retained deterministic trajectory checkpoints; it does not "
                "gate intermediate FIT movement"
            ),
            "b_solve": (
...
def main():
    p = argparse.ArgumentParser(
        description=(
            "EngineV2 North Star v3 search-robust experiment. Keeps v2 evidence, "
            "objective and DSP architecture unchanged while adding deterministic "
    ...
    p.add_argument(
        ...
        help=(
            "Maximum coarse-to-fine search steps per start. Search can stop sooner "
            "after both general step floors have been used."
        ),
    )
    ...
    p.add_argument(
        "--multistarts",
        type=int,
        default=DEFAULT_MULTISTARTS,
        help=(
            f"Deterministic P/K starting points (1-{MAX_MULTISTARTS}); "
            f"default {DEFAULT_MULTISTARTS}."
        ),
    )
    ...
    print("NORTH STAR v3 canonical probes (unchanged from v2):")
